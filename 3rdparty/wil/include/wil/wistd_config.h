...
#  if defined(__WI_LIBCPP_COMPILER_GCC) && __WI_GNUC_VER >= 800
// gcc 8+ renamed the builtin from __is_literal to __is_literal_type
...
// __is_convertible_to(TFrom, TTo) is a clang/MSVC builtin that gcc does not
// provide.  gcc instead provides __is_convertible(TFrom, TTo) (no _to suffix)
// as a 2-arg builtin with the same semantics.  Map the name across so WIL's
// extensive use of __is_convertible_to in <wil/com.h> compiles under gcc.
// Note: do NOT define this for clang -- clang has __is_convertible_to natively.
#if defined(__WI_LIBCPP_COMPILER_GCC) && !defined(__is_convertible_to)
#  define __is_convertible_to(__from, __to) __is_convertible(__from, __to)
#endif
...
{
     // clang has the __nullptr keyword as an alias for nullptr; gcc and other
     // compilers do not.  Fall back to plain nullptr when not on clang.
#if defined(__WI_LIBCPP_COMPILER_CLANG)
