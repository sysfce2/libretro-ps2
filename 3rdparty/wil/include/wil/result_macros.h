...
// MinGW's <winnt.h> only ships the basic Interlocked* aliases (Acquire,
// Release, plain).  The NoFence variants are an MSVC-only addition.  On
// x86/x64 every Interlocked operation already implies a full barrier, so
// aliasing the NoFence variants to the regular ones is functionally
// correct (it just gives up the ARM64 perf hint).  This lets WIL parse
// under gcc/MinGW.
#if !defined(_MSC_VER)
#  ifndef InterlockedIncrementNoFence
#    define InterlockedIncrementNoFence(p) InterlockedIncrement(p)
#  endif
#  ifndef InterlockedDecrementNoFence
#    define InterlockedDecrementNoFence(p) InterlockedDecrement(p)
#  endif
#  ifndef InterlockedExchangeNoFence
#    define InterlockedExchangeNoFence(p, v) InterlockedExchange(p, v)
#  endif
// WriteRelease / ReadAcquire are MSVC-only intrinsics for atomic store/load
// with release/acquire semantics, declared in MSVC's <winnt.h>.  On x86/x64
// a plain volatile store is a release and a plain volatile load is an
// acquire, so the aliases are functionally correct.  On ARM64 this would
// drop the explicit memory ordering hint (gcc would have to be told via
// __atomic_store_n / __atomic_load_n with the right memory_order), but
// MinGW on ARM64 isn't a target we care about for this build.
#  ifndef WriteRelease
...
#if defined(_MSC_VER)
        // MSVC build: use the in-tree replicas as before.  In particular,
        // STRSAFEAPI on MSVC's <strsafe.h> is just '__inline HRESULT WINAPI'
        // (no extern "C") so 'static STRSAFEAPI ...' parses fine.
...
#define STRSAFE_USE_SECURE_CRT 1
...
#else // !_MSC_VER
        // MinGW build: STRSAFEAPI on mingw's <strsafe.h> is
        //   'extern "C" __inline HRESULT WINAPI'
        // and 'static extern "C"' is a linkage clash, plus several SAL types
        // such as STRSAFE_PCNZCH aren't defined on mingw.  mingw already
        // ships StringCchLengthA / StringCchPrintfA in <strsafe.h>, so just
        // forward to the global versions instead of replicating them here.
        inline HRESULT StringCchLengthA(_In_ PCSTR psz, _In_ size_t cchMax, _Out_opt_ size_t* pcchLength)
        {
            return ::StringCchLengthA(psz, cchMax, pcchLength);
        }

        inline HRESULT StringCchPrintfA(_Out_writes_(cchDest) _Always_(_Post_z_) char* pszDest, _In_ size_t cchDest, _In_ _Printf_format_string_ const char* pszFormat, ...)
        {
            va_list argList;
            va_start(argList, pszFormat);
            HRESULT hr = ::StringCchVPrintfA(pszDest, cchDest, pszFormat, argList);
            va_end(argList);
            return hr;
        }
#endif // _MSC_VER
        ...
        _Ret_range_(pStart, pEnd) inline unsigned char *GetResultString(_In_reads_to_ptr_opt_(pEnd) unsigned char *pStart, _Pre_satisfies_(pEnd >= pStart) unsigned char *pEnd, _Out_ TString *ppszBufferString)
        {
            ...
            *ppszBufferString = (cchLen > 0) ? reinterpret_cast<TString>(pStart) : nullptr;
            // NOTE: was 'min(pEnd, ...)' which relied on Windows.h's 'min' macro.
            // With NOMINMAX defined (or on toolchains where Windows.h does not
            // unconditionally provide it), spell out the min explicitly so we
            // don't drag in <algorithm> or fight the NOMINMAX policy.
            unsigned char *pCandidate = pStart + ((cchLen + 1) * sizeof((*ppszBufferString)[0]));
            auto pReturn = (pCandidate < pEnd) ? pCandidate : pEnd;
